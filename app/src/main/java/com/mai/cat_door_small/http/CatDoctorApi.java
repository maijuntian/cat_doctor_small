...
public class CatDoctorApi extends BaseRetrofitService<CatDoctorService> {
    ...
    protected String getBaseUrl() {
        return "http://193.112.79.134:8080/";
...
//        return "http://dev-apisail.healthmall.cn/api/";
    ...
    protected <M> Observable<M> checkNoResult(Observable<M> observable, Context context) {
        return checkError(checkNetWork(observable.subscribeOn(Schedulers.newThread()).observeOn(AndroidSchedulers.mainThread()), context), context);
    }
    ...
    public Observable<PicUrl> faceTonUpload2(final Bitmap bmp, final int testType, final Context ctx) {
        return checkNoResult(Observable.just(bmp).map(new Func1<Bitmap, File>() {
            @Override
            public File call(Bitmap bitmap) {
                String path;
                if (testType == 1) {
        ...
        }).flatMap(new Func1<File, Observable<PicUrl>>() {
            @Override
            public Observable<PicUrl> call(File file) {

                return mService.faceTonUpload2(new MParams().add("img", file).getFileParams());
            }
        }), ctx);
    }
